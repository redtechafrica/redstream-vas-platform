...
const Subscriptions = () => {
  ...
  const handleSearch = (value: string) => {
    ...
    const filtered = subscriptionsData.filter(sub => 
      sub.subscriber.toLowerCase().includes(value.toLowerCase()) ||
      sub.services.toLowerCase().includes(value.toLowerCase()) ||
      sub.status.toLowerCase().includes(value.toLowerCase())
    )
      ...
      <div className="flex-1 space-y-4 p-8 pt-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-muted-foreground">Showing 1 to {filteredData.length} of {subscriptionsData.length} records</span>
          </div>
